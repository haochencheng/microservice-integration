<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="断言名称" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="断言参数" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.predicateDefinitionArgs }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="断言名称" prop="name" >
          <el-select v-model="temp.name" class="filter-item" placeholder="请选择断言名称">
            <el-option v-for="item in temp.nameList" :key="item" :label="item" :value="item"/>
          </el-select>
        </el-form-item>
        <el-form-item label="断言参数" prop="predicateDefinitionArgs" placeholder="多个参数逗号隔开">
          <el-input v-model="temp.predicateDefinitionArgs"/>
        </el-form-item>
...
<script>
import { fetchList, enabledById, createPredicate, updatePredicate } from '@/api/predicateDefinition'
...
export default {
  ...
  data: function() {
    return {
      ...
      temp: {
        ...
        name: 'Path',
        routeDefinitionId: '',
        predicateDefinitionArgs: '',
        enabled: true,
        nameList: ['Path'],
        desc: ''
      },
      rules: {
        name: [{ required: true, message: '请填写断言名称', trigger: 'change' }],
        routeDefinitionId: [{ required: true, message: '请选择所属路由', trigger: 'change' }],
        predicateDefinitionArgs: [{ required: true, message: '请填写断言参数', trigger: 'change' }]
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    handleUpdateServiceEnabled(id, status) {
      this.$confirm(`确认${msgMap[status]}该断言吗？`)
        .then(() => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createPredicate(this.temp).then(() => {
